export const PRODUCT_008 = {
  id: '008',
  title: 'No-Code AI Automation Guide',
  subtitle: 'Automate Your Repetitive Tasks With AI — No Programming Skills Required',
  type: 'guide' as const,
  niche: 'automation',
  targetAudience: 'small business owners, solopreneurs, virtual assistants, office professionals',
  pages: 40,
  wordCount: 12000,
  fileFormats: ['PDF'],
  price: 12.99,
  chapters: [
    {
      number: 1,
      title: 'The Automation Mindset: What to Automate First',
      content: `Before you touch a single tool, you need to know what to automate. Most people make the mistake of automating the wrong things first — they chase the most technically interesting workflows rather than the ones that will give them back the most time.
...
Most automations take 1-3 hours to set up. At $2,600/year, that is a 650x ROI in Year 1. This is why automation is one of the highest-leverage activities for any knowledge worker.`,
    ...
    {
      number: 2,
      title: 'Your No-Code Automation Toolkit',
      content: `You do not need to code. Every tool in this chapter works through a visual, drag-and-drop interface. Here is an honest assessment of each platform and when to use it.
...
Pricing: Free (5 Zaps, 100 tasks/month), Starter ($19.99/month, 20 Zaps, 750 tasks), Professional ($49/month, unlimited Zaps, 2,000 tasks). The free tier is sufficient for testing; Professional handles a serious solopreneur workload.
Limitations: Slower than Make.com for complex multi-step workflows; costs scale quickly with volume.

**Make.com (make.com, formerly Integromat)**
...
Pricing: Free (1,000 operations/month), Core ($9/month, 10,000 ops), Pro ($16/month, 10,000 ops + advanced features). Significantly cheaper than Zapier at scale.
Limitations: Slightly steeper learning curve; documentation can be sparse for advanced features.
...
Pricing: Free (limited), Pro $2.50/month (multi-action applets).
...
How it works: AI add-on ($10/month) integrated into your Notion workspace. Summarizes pages, generates action items, drafts content, and analyzes data within your notes.
When to use it: Not an automation platform per se — it is AI-augmented note-taking. Combine it with Zapier or Make for full automation workflows.

**Airtable Automations (airtable.com)**
Best for: Teams managing structured data (CRMs, project trackers, inventory).
How it works: Built-in automation layer that triggers on database events — new record, field change, date arrival. Connect to external apps via webhook or native integrations.
Pricing: Free (limited runs), Plus ($10/seat/month, 5,000 runs/month), Pro ($20/seat/month, 50,000 runs).
When to use it: When your automation needs are centered around a database, Airtable's built-in automations are faster to set up than building a Zapier integration.
...
| Need AI text generation in your Zap | Zapier + OpenAI step |`,
    ...
    {
      number: 3,
      title: '5 Automation Recipes You Can Set Up Today',
      content: `Each of these recipes takes under 30 minutes to set up with a free account. Follow the steps exactly — every field name and setting is included.

**Recipe 1: Email → Notion Database (Capture Important Emails to Task List)**
...
Common errors: "Cannot find Notion database" — make sure you shared the database with your integration in Step 3. "Gmail trigger not firing" — labels are case-sensitive; make sure the Zap label matches exactly.
...
Pro tip: Add a third action — Gmail → Send Email — to auto-send a confirmation email to the person who submitted the form. This saves you a manual follow-up step.
...
Note: Platform character limits apply — add a text truncation step for Twitter (280 chars) vs LinkedIn (3,000 chars). Buffer handles image scheduling if you provide an image URL.

**Recipe 5: Automated Invoice Follow-Up**

Overview: When an invoice in QuickBooks or FreshBooks becomes overdue, automatically send a polite follow-up email at day 3, day 7, and day 14.

Tools needed: QuickBooks Online or FreshBooks, Zapier (Pro), Gmail

Steps:
1. In Zapier: Trigger = QuickBooks → New Invoice
2. Add a Delay step (available on Zapier Starter): Delay For → 3 days
3. Check current invoice status with a QuickBooks "Find Invoice" step
4. Add a Filter: Only Continue If → Invoice Status = Overdue
5. Action: Gmail → Send Email → to client email from invoice → Subject: "Quick reminder: Invoice #[number] is due" → Body: "Hi [Client Name], just a friendly reminder that Invoice #[number] for $[amount] was due on [date]. If you have any questions or have already sent payment, please disregard. [Your name]"
6. Duplicate the path for Day 7 and Day 14 with escalating urgency in the message
7. Test with a dummy invoice

This single automation recovers an average of $1,200-$3,000 per year for freelancers and small businesses who previously let invoices slip.`,
    },
    {
      number: 4,
      title: 'Adding AI to Your Automations',
      content: `Basic automations move data from one place to another. AI-enhanced automations move data from one place to another AND intelligently transform it along the way. The difference between a basic Zap and an AI-enhanced workflow is the difference between a mail sorter and a smart assistant who reads your mail and decides what to do with it.
...
**Using OpenAI/ChatGPT API in Zapier:**

In Zapier, add an "OpenAI" action step (available on Starter plan and above). Select the "Send Prompt" action. The key fields:
...
**Using OpenAI in Make.com:**

Add an "OpenAI" module. Use the "Create a Completion" action. Cost per AI call in Make.com with gpt-4o-mini: approximately $0.001-0.003 per call — essentially free at automation scale.

**5 AI-Enhanced Automation Templates with Full Instructions:**
...
Workflow:
1. Trigger: Notion → New Page in "Meeting Notes" database (or Google Drive → New File in folder)
...
4. Action 2: For each action item extracted, create a new task in your project management tool (Asana, Todoist, etc.)
...
2. AI Step: OpenAI → "This is a customer support email for a [your business type]. Read the email and write a professional, helpful response that: addresses their specific question, maintains a warm but professional tone, and ends with a clear next step. If you cannot answer definitively (need more info), write a response that acknowledges their query and asks 1 clarifying question. Email: [email body]. Our standard policy is [your brief policy]. Return only the email response text."
...
2. Filter: Only continue if mention sentiment is neutral or negative (these need responses most)
...
2. Conditional check: Which form/landing page did they subscribe from? (maps to their interest)
...
4. Action: Send via ConvertKit or Mailchimp as immediate triggered email`,
    ...
    {
      number: 5,
      title: 'Building a Lead Capture & Nurture Machine',
      content: `This chapter walks through building a complete, end-to-end lead automation system using Airtable as your CRM, Make.com as your automation engine, and ChatGPT for AI personalization. By the end, every new lead gets: immediate CRM entry, an AI-personalized welcome email, a 5-touch follow-up sequence, and a sales alert to your team — all automatically.
...
Module 4B: OpenAI → Create Completion → Prompt: "Write a personalized welcome email for [Name] from [Company] who found us through [Source]. They are interested in [product/service]. The email should: be warm and specific to their situation, provide one immediately useful insight about [their industry/topic], and invite them to a 15-minute call with a Calendly link placeholder. Keep under 200 words. Return only the email body."
Module 5B: Gmail (or SendGrid) → Send Email → to Lead email → subject: "Welcome, [Name] — let's make this valuable" → body: AI output from previous step

**Step 3: The 5-Day Follow-Up Sequence**

Use Airtable Automations for the follow-up sequence (or Make.com with delay modules):

Day 1 (after welcome): "One thing most [industry] professionals miss about [topic]" — educational email with genuine insight
Day 3: "Quick question, [Name]" — 2-sentence personal email asking one specific question about their situation
...
Day 10: Final follow-up — acknowledge no response, keep door open, CTA for when timing is right

Each email in the sequence can be AI-personalized by including the lead's company and source in the AI prompt for each step.

**Step 4: Lead Scoring Automation**

Automatically increase lead score based on engagement signals:
- Opens welcome email: +10 points (use email open tracking)
- Clicks a link: +15 points
- Replies to any email: +25 points
- Books a call: +50 points

Use Airtable Automations to update the Lead Score field when these events are detected. When Lead Score reaches 50+, trigger an immediate Slack alert to the sales team: "High-priority lead: [Name] has engaged multiple times. Review now."

**Estimated Setup Time:** 3-4 hours total.
**Estimated ROI:** If this system generates 2 additional qualified calls per month at a $300 average deal value, payback is immediate. Most users see 3-5x more follow-through on new leads compared to manual systems.`,
    },
    {
      number: 6,
      title: 'Content Creation Automation',
      content: `Content creation is one of the highest-value activities for building an audience and driving inbound leads — and also one of the most time-consuming. The automations in this chapter create a content engine where you provide the seed idea and the system handles distribution, repurposing, and scheduling.
...
1. Write one blog post in your CMS (WordPress, Ghost, Webflow, or Notion)
2. Tag it as "Ready to Repurpose"
3. Automation detects the tag → sends to AI for repurposing → creates all derivative content in a content calendar
4. You review and approve → Buffer or Hootsuite schedules all posts
...
- Module 2: Notion → Get Page → retrieve full page content (article body)

Step 3: Add 5 parallel AI repurposing modules (using Make.com Router):

Branch 1 — Twitter Thread:
OpenAI prompt: "Turn this blog post into a Twitter thread of exactly 5 tweets. Tweet 1 should hook the reader with the most surprising insight. Tweets 2-4 should expand the main points. Tweet 5 should be a CTA. Each tweet under 280 characters. Separate tweets with [---]. Blog post: [content]"

Branch 2 — LinkedIn Post:
OpenAI prompt: "Rewrite this blog post as a LinkedIn post. Start with a bold first line that creates a scroll-stopping hook. Use short paragraphs (1-2 sentences max). Include one personal observation or story. End with a question to drive comments. 250-350 words. Blog post: [content]"

Branch 3 — Instagram Caption:
OpenAI prompt: "Write an Instagram caption based on this blog post. Lead with an emotion or curiosity hook. Share 2-3 key insights in bullet form with emoji. End with a question or CTA. Include 20 relevant hashtags at the bottom separated by line break. Under 300 words. Blog post: [content]"

Branch 4 — Email Newsletter:
OpenAI prompt: "Turn this blog post into an email newsletter section. Subject line options (provide 3). Preview text. Body (200 words max): start with why this matters to the reader, share the core insight, link to the full article. Use a conversational tone. Blog post: [content]"

Branch 5 — Video Script (Short Form):
OpenAI prompt: "Write a 60-second video script based on this blog post. Hook (first 3 seconds): one sentence that creates immediate curiosity. Body (30 seconds): the single most valuable insight, explained simply. CTA (5 seconds): tell viewers to follow for more. Include [B-ROLL suggestions in brackets]. Blog post: [content]"
...
**Automated Blog Outline Generation:**

For writers who get stuck on outlines, this workflow generates a complete SEO-optimized outline from just a topic keyword:

1. Trigger: New row in Google Sheets "Blog Ideas" with Status = "Outline Requested"
2. AI Step: OpenAI → "Create a detailed SEO-optimized blog post outline for the keyword: [keyword]. Include: suggested title (include keyword, under 60 chars), meta description (under 160 chars), H2 headings (5-7), sub-points under each H2 (2-3 bullet points), suggested internal link topics, and a suggested CTA. Format as structured markdown."
3. Action: Add outline to the same Google Sheets row in the "Outline" column
4. Send email notification: "Your outline for '[keyword]' is ready"
...
**Estimated time saved:** 2-3 hours per blog post published.`,
    ...
    {
      number: 7,
      title: 'Your 30-Day Automation Roadmap',
      content: `Automation is a compounding skill. The first automation takes the longest and saves the least. By your tenth automation, each one takes 20 minutes to set up and saves hours per month. This roadmap builds your skills and your systems at the same time.
...
Day 5-7: Test your automation with real-world data. Run 5-10 test triggers. Check: does the trigger fire reliably? Does the data map correctly? Are there any error notifications?
...
Day 15-18: Build or extend your automation to write data into a central system — your CRM (Airtable, HubSpot, Notion), your task manager, or your spreadsheet. The goal this week is to have automation feeding the tools you already use daily.

Day 19-21: Set up the Lead Capture & Nurture Machine from Chapter 5. This is the highest-ROI automation in this guide. Even a simplified version — form submission → CRM entry → welcome email — recovers 2-3 hours per week.
...
Day 26-28: Pick your next automation. Use what you learned from Weeks 1-3. Your second automation should be faster to build.
